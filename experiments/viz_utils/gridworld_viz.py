...
def run_animation(horizon: int = 10):
    ...
    agents = [
        Agent(env, mode="recon",    seed=0, start=(env.H//2, env.W//2), horizon=horizon),
        Agent(env, mode="humphrey", seed=1, start=(env.H//2, env.W//2), horizon=horizon),
        Agent(env, mode="humphrey_barrett", seed=2, start=(env.H//2, env.W//2), horizon=horizon),
    ]
    ...
    ax_ns  = fig.add_subplot(gs[1, :2])  # Only span first 3 columns
    ax_hud = fig.add_subplot(gs[2, :2])  # Only span first 3 columns
    ...
    info_texts = []  # Store text objects for updates
    for i, ax in enumerate(ax_env):
        im = ax.imshow(bg, vmin=-1, vmax=1, extent=[-0.5, env.W-0.5, env.H-0.5, -0.5])
        ims.append(im)
        ...
        # Add text object below the gridworld
        info_text = ax.text(
            0.5,
            -0.05,
            "",
    ...
    def update(frame):
        for i, ag in enumerate(agents):
            ...
            pts = cone_polygon(ag.x, ag.y, ag.heading, radius=5, fov_deg=70)
            cones[i].set_xy(pts)
            ...
            info_texts[i].set_text(
                f"act={action}  Ns={ns:.2f}  touch={i_t:.2f}\nsmell={i_s:.2f}  vision={i_v:.2f}"
            )
