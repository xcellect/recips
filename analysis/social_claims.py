...
def build_social_artifacts(
    *,
    ...
    headline_summary = summarize_by_condition(
        headline,
        ["help_rate_when_partner_distressed", "mutual_viability", "rescue_latency", "self_cost_of_help", "episode_joint_longevity"],
    )
    headline_summary.to_csv(os.path.join(outdir, "headline_summary.csv"), index=False)
    lesion_summary = summarize_by_condition(
        lesion,
        ["help_rate_when_partner_distressed", "mutual_viability", "rescue_latency"],
    )
